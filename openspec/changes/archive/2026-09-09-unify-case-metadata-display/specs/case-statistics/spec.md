...
#### Scenario: 案件狀態比例配色
- **WHEN** 檢視 Dashboard 的「案件狀態比例 (期別內)」區且 `statusRatio` 含 `PENDING/RESOLVED/CLOSED`
- **THEN** 三條 `progress-bar` 的背景色分別為 `#f59e0b`（待處理）、`#10b981`（已診斷）、`#64748b`（已結案），語意不變僅色調優化
