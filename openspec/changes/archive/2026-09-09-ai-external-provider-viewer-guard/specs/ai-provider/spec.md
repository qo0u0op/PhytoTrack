...
系統 SHALL 支援 `ai.provider = "local" | "external"`（`phytotrack.toml`），`local` 指向 `http://localhost:11435`，`external` 指向任意 OpenAI 相容 `base-url` 並使用對應 `model`/`api-key`；預設 `local`，切換無需改程式。
