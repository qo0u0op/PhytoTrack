...
- [x] 2.2 刪除 `service/AuthService.java:165` 的 `updateRole(String)` 相容多載 — 評估保留，屬簡單委派且 `UserAdminControllerTest` 仍依賴，確認保留不刪除，視為完成（可選）
...
- [x] 4.1 `service/InputSanitizer.java:10` 遷至 `util/validation/InputSanitizer`（保留雙層 `fieldLabel` 多載），`SenderService.java:108` / `CaseService.java:196` 與 `AccountService/AuthService` 同步改 `import com.d0w0b.phytotrack.util.validation.InputSanitizer`，舊路徑已刪除，`mvn test` 通過
- [x] 4.2 更新 `docs/notebook/security/pentest-2026-09-04.md:6` 踩坑備註：已清可選注入債務，`AGENTS.md:60` ADR 清單已補 `012/013`

## 5. 驗證與封存

- [ ] 5.1 每步後執行 `mvn test` 與 `openspec validate --specs --changes`，全程綠燈後 `openspec archive reduce-complexity-2026-09-05 -y` — 已 `mvn test 154 passed`、`openspec validate 22 passed`，待最終封存
