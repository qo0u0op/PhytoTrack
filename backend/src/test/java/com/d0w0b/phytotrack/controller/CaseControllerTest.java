...
class CaseControllerTest {
  ...
  private static CaseResponse sampleResponse () {
    return new CaseResponse (1L, LocalDate.of (2026, 8, 18), "2 分地", "約 3 成",
        "葉片出現斑點", null, "PENDING",
        LocalDateTime.now (), LocalDateTime.now (),
        1L, "張三", "阿明", "0912345678", "測試路 1 號", 1L, "霧峰區", "臺中市", 1L, "農民",
        1L, "霧峰區", "臺中市", "糧食作物",
        "水稻", "露天", "診斷", "送件",
  ...
  void list_shouldReturnPageForAnyAuthenticatedUser () throws Exception {
    when (caseService.list (any (), any ())).thenReturn (new PageImpl<>(List.of (new CaseSummaryResponse (1L, LocalDate.of (2026, 8, 18), "水稻", "張三", "阿明", "0912345678",
  ...
  @Test
  @WithMockUser (roles = "VIEWER")
  void export_shouldForbidViewer () throws Exception {
    mockMvc.perform (get ("/api/cases/export"))
  ...
  void list_withInvalidStatus_shouldReturnBadRequest () throws Exception {
    when (caseService.list (any (), any ())).thenThrow (new ApiException ("INVALID_STATUS", HttpStatus.BAD_REQUEST, "無效的狀態：DRAFT"));
