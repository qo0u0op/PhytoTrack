...
- 每能力一個獨立 OpenSpec change（spec 已在主規格，`skip_specs: true`），建議順序：case-search（已交付）→ case-lifecycle → case-statistics → case-report → user-admin → reference-data-admin → sender-management → ops-backup
...
- **建議順序**：case-search（已交付）→ case-lifecycle → case-statistics（應於 case-lifecycle 後實作，避免重做對映）→ case-report → user-admin → reference-data-admin → sender-management → ops-backup
