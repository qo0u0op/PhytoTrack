...
- 每能力一個獨立 OpenSpec change（spec 已在主規格，`skip_specs: true`），建議順序：case-search（已交付）→ case-lifecycle（已交付）→ case-statistics（已交付）→ case-report（已交付）→ user-admin → reference-data-admin → sender-management → ops-backup
...
- **建議順序**：case-search（已交付）→ case-lifecycle（已交付）→ case-statistics（已交付）→ case-report（已交付）→ user-admin → reference-data-admin → sender-management → ops-backup
