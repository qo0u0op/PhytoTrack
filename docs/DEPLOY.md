...
## 8. CSV 匯出欄位順序變更 (BREAKING，2026-09-01)

本版本依 `docs/diagnoses.typ` 紙本邏輯重排 `GET /api/cases/export` 的欄位順序，對齊 `align-case-field-order` 變更 (Q1-Q5 定版)。舊版以欄位索引解析者請改以表頭解析。
...
|---|---|---|
| 案件編號 | 收件編號 | 更名，按表單 |
| 縣市鄉鎮 | 病蟲害發生地點_縣市/鄉鎮 | 由送件人地址改為田區位置 |
| 身分別 | 送件人身分別 | 同義 |
| 作物 | 作物種類 + 作物名稱 | 新增種類 (Q5) |
| 病蟲害 | 病害/蟲害/有害動物/生理因子/其他 五欄 | 拆分，`pestNote` 以 `名稱 (備註)` 呈現 |
| 防治建議 | 建議事項 | 更名，第 6 項「其他」→「其他回覆」 (Q4) |
| - | 被害描述 | 新增，`pest_note` 串接 (Q1) |
| - | 是否同寄件人 | 新增，田區同寄件人標記 |

影響：下游若以索引 (column index) 解析 CSV，需改以表頭名稱解析；`hints` 第 6 項顯示已由「其他」更名為「其他回覆」。
